...
class ConditioningInputView extends StatefulWidget {
  final String goal;

  const ConditioningInputView({Key? key, required this.goal}) : super(key: key);

  @override
  _ConditioningInputViewState createState() => _ConditioningInputViewState();
}

class _ConditioningInputViewState extends State<ConditioningInputView> {
        ...
        if (_formKey.currentState!.validate()) {
          ...
          if (user != null) {
            try {
              ...
              if (_days != null) {
                await Provider.of<ConditioningViewModel>(context, listen: false).generatePlan(
  ...
  ElevatedButton _buildDeleteButton(UserModel? user) {
    return ElevatedButton(
      onPressed: () async {
        if (user != null) {
          try {
            await Provider.of<ConditioningViewModel>(context, listen: false)
